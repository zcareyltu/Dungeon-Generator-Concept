...
function mask_cells(dungeon, mask){
    ...
    for (var r = 0; r <= dungeon['n_rows']; r++) {
        for (var c = 0; c <= dungeon['n_cols']; c++) {
            if(!(mask[r * r_x][c * c_x])) cell[r][c] = BLOCKED;
        }
    }
    return dungeon;
}
...
function pack_rooms(dungeon){
    ...
    for (var i = 0; i < dungeon['n_i']; i++) {
        ...
        for (var j = 0; j < dungeon['n_j']; j++) {
            ...
            if((i == 0 || j == 0) && Math.floor(random(2))) continue;
...
function open_room(dungeon, room) {
    ...
    for (var i = 0; i < n_opens; i++) {
        var sill = splice(list, Math.floor(random(/*@list*/1)),1);
        ...
        if (out_id) {
            var connect = join(',',(sortById(room['id'], out_id)));
    ...
    // open door
        ...
        if(door) room[door][open_dir] = room[door][open_dir].push(door);
    }
...
function alloc_opens(dungeon, room){
    ...
    var flumph = Math.floor(sqrt(room_w * room_h));
    var n_opens = flumph + Math.floor(rand(flumph));
  
    return n_opens;
}

function door_type(){
    var i = Math.floor(rand(110));
...
function tunnel_dirs(dungeon, last_dir){
    var p = corridor_layout[$ungeon['corridor_layout']];
    var dirs = shuffle(dj_dirs);
  
    if (last_dir && p) {
        if (Math.floor(rand(100)) < p) unshift(dirs, last_dir);
    }
    return dirs;
}
...
function collapse_tunnels(dungeon, p, xc){
    ...
    for (var i = 0; i < dungeon['n_i']; i++) {
        ...
        for (var j = 0; j < dungeon['n_j']; j++) {
            ...
            if(!(all || (Math.floor(rand(100)) < p))) continue;
...
function set_room(dungeon, proto){
    ...
    if(!(proto['height'])) {
        if (proto['i']) {
            ...
            proto['height'] = Math.floor(rand(r)) + base;
        } else {
            proto['height'] = Math.floor(rand(radix)) + base;
        }
    }
    if (!proto['width']) {
        if (proto['j']) {
            ...
            proto['width'] = Math.floor(rand(r)) + base;
        } else {
            proto['width'] = Math.floor(rand(radix)) + base;
        }
    }
    if (!proto['i']) {
        proto['i'] = Math.floor(rand(dungeon['n_i'] - proto['height']));
    }
    if (!proto['j']) {
        proto['j'] = Math.floor(rand(dungeon['n_j'] - proto['width']));
    }
    return proto;
}
...
function collapse(dungeon, r, c, xc){
    ...
    for (var dir in Object.keys(xc)) {
        if (check_tunnel(cell, r, c, xc[dir])) {
            for (var p in xc[dir]['close']) {
...
function displayDungeon(dungeon){
    ...
    paintDungeon(canvas, ctx, dungeon);
...
function paintDungeon(canvas, g, dungeon){
    var image = scale_dungeon($dungeon);

    //# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
    //# new image
    canvas.width = image['width'];
    canvas.height = image['height'];
    var pal = get_palette(image, canvas);
    image['palette'] = pal;
    var base = base_layer(dungeon, image, canvas);
...
function open_cells(dungeon, image, g) {
    ...
    for (var r = 0; r <= dungeon['n_rows']; r++) {
        ...
        for (var c = 0; c <= dungeon['n_cols']; c++) {
            ...
            filledRectangle(g, x1, y1, dim+1, dim+1, pal['white']); //$ih->copy($base,$x1,$y1,$x1,$y1,($dim+1),($dim+1));
        }
    }
    return g;
  }
...
function base_layer(dungeon, image, canvas, g){
    var max_x = image['max_x'];
    var max_y = $image['max_y'];
    var dim = image['cell_size'];
    var pal = image['palette'];
    ...
    /*if (tile = pal['open_pattern']) {
        ...
        filledRectangle(g, 0, 0, max_x, max_y, pal['white']); //$ih->filledRectangle(0,0,$max_x,$max_y,$pal->{'white'});
        fill(g, 0, 0, pal['white']); //$ih->fill(0,0,$pal->{'white'});
    }
    ...
    if (tile = pal['background']) {
        ...
        filledRectangle(g, 0, 0, max_x, max_y, pal['white']); //ihh->filledRectangle(0,0,$max_x,$max_y,$pal->{'white'});
    }
...
function get_palette(image, canvas){
    var pal; 
...
function splice(array, offset, length){
    array.slice().splice(offset, length);
    return array;
}
